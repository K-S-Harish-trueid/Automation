...
SHEET_DOB_MISTAKES = "DOB Corrections"
...
# ---- Flow 3: three-file workflow (CMS Mobile, CMS Card, Haider Stage 2) ----

# CMS Mobile Numbers file: ACCOUNT_NUMBER + PHONE_NUMBER (flat table, no sheets)
CMS_MOBILE_COLS = MOBILE_COLS  # ["PHONE_NUMBER"]

# CMS Card Details file: ACCOUNT_NUMBER + CMS_UPDATE_COLS (flat table, no sheets)
...
CMS_CARD_COLS = CMS_UPDATE_COLS  # ["CARD_NUMBER", "ACCOUNT_TYPE", ...]

# Haider Stage 2 Corrected file: three-sheet workbook
HAIDER_STAGE2_SHEETS = {
...
def _normalise_date_opened(resp: pd.DataFrame) -> pd.DataFrame:
    ...
    # Reformat only the values that parsed successfully
    valid = parsed.notna()
...
def apply_cms_mobile_response(df: pd.DataFrame, df_mobile: pd.DataFrame):
    ...
    Accounts that are absent from the CMS file (or have a blank/zero number
    ...
    # Any account not covered by the CMS file will still have a blank /
    # all-zeros PHONE_NUMBER.  Stamp those with the standard sentinel so
    # they are clearly flagged rather than silently carrying a zero.
...
def validate_haider_stage2_response(sheets: dict[str, pd.DataFrame] | None):
    """Validate Haider's Stage 2 corrected workbook (3-sheet: Name/ID/DOB)."""
    _require_sheets(sheets, HAIDER_STAGE2_SHEETS,
                    missing_msg="Haider's Stage 2 corrected file is required")


def apply_haider_stage2_response(df: pd.DataFrame, sheets: dict[str, pd.DataFrame]):
    """Merge Name Validation, ID Corrections, and DOB Corrections sheets from
    Haider's Stage 2 corrected workbook into df."""
    validate_haider_stage2_response(sheets)
    name_matched = _overwrite_nonblank(df, sheets[SHEET_NAME_VALIDATE], NAME_COLS)
    id_matched = _overwrite_nonblank(df, sheets[SHEET_ID_CORRECTIONS], ID_COLS)
    dob_matched = _overwrite_nonblank(df, sheets[SHEET_DOB_MISTAKES], DOB_COLS)
    summary = (
        f"Applied Haider's Stage 2 corrections: {name_matched} name, "
