...
def get_status(job_id: str) -> dict:
    ...
    for s in status.get("stages", []):
        if s.get("id") in stage_title_map:
            s["title"] = stage_title_map[s["id"]]
...
def list_job_summaries(stage_id: str | None = None) -> list[dict]:
    ...
    for job_dir in JOBS_DIR.iterdir():
        ...
        from .pipeline.registry import STAGES
        stage_title_map = {s["id"]: s["title"] for s in STAGES}
        cur_stage_id = stage["id"] if stage else "done"
        cur_stage_title = stage_title_map.get(cur_stage_id, stage["title"] if stage else "Final Output")
