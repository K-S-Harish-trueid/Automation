...
def _current_stage(status: dict) -> dict | None:
    ...
    if idx >= len(status["stages"]):
        return None
    stage = dict(status["stages"][idx])
    from .pipeline.registry import STAGES
    stage_title_map = {s["id"]: s["title"] for s in STAGES}
    if stage.get("id") in stage_title_map:
        stage["title"] = stage_title_map[stage["id"]]
    return stage
...
def _public_job_status(status: dict) -> dict:
    ...
    if "stages" in public_status:
        ...
        for s in public_status["stages"]:
            sc = dict(s)
            if sc.get("id") in stage_title_map:
                sc["title"] = stage_title_map[sc["id"]]
            stages_copy.append(sc)
        public_status["stages"] = stages_copy
...
def _write_filtered_sheet(writer, sheet_name: str, df: pd.DataFrame, mask: pd.Series, cols: list[str], reasons_fn, include_notes: bool = False):
    ...
    if VALIDATION_NOTES or include_notes:
...
def _write_flow1_haider_xlsx(df: pd.DataFrame, out_path) -> None:
    """Flow 1 dispatch file for Haider: Mobile sheet cut down to
    ...
    validation is deferred to Stage 2."""
...
def _write_ids_and_dob_xlsx(df: pd.DataFrame, out_path) -> None:
    ...
    output for Haider: 2 sheets, ID Corrections + DOB Mistakes."""
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        _write_filtered_sheet(
...
def _write_flow1_naresh_xlsx(df: pd.DataFrame, out_path) -> None:
    """Flow 1 dispatch file for Naresh: ID Corrections + DOB Mistakes sheets,
    cut down to rows with MISSING OR INVALID IDs/DOBs so Naresh can correct
    both gap and format errors in one pass."""
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        _write_filtered_sheet(
            writer, flow_merge.SHEET_ID_CORRECTIONS, df, pipeline.mask_id_only_invalid(df),
            flow_merge.ID_COLS, pipeline.validation_reasons_id_only,
...
def _write_flow2_haider_xlsx(df: pd.DataFrame, out_path) -> None:
    """Flow 2 dispatch file for Haider: Name Validation, ID Corrections + DOB Mistakes sheets,
    containing all invalid and remaining-missing rows after Naresh's updates.
    DOB Mistakes sheet includes validation remarks explaining why each row is flagged.
    In the Name Validation sheet ACCOUNT_LAST_NAME is placed as the second column
    (immediately after ACCOUNT_NUMBER) for easier review."""
    # Column order for the Name Validation sheet: LAST_NAME as second column.
    name_cols_ordered = [
        "ACCOUNT_LAST_NAME",
        "ACCOUNT_FIRST_NAME",
        "ACCOUNT_MIDDLE_NAME",
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        _write_filtered_sheet(
            ...
            name_cols_ordered, pipeline.validation_reasons_name,
        )
        _write_filtered_sheet(
            writer, flow_merge.SHEET_ID_CORRECTIONS, df, pipeline.mask_id_only_invalid(df),
            flow_merge.ID_COLS, pipeline.validation_reasons_id_only,
...
def _write_flat_xlsx(df: pd.DataFrame, out_path) -> None:
    ...
    SMART_IDENTIFIER is dropped here: it is used only as an internal reference
    for Naresh and is not needed in the final delivered dataset."""
